...
try {
    ...
    Assert(InStr(voice.ui.xaml, 'Height="268"') && InStr(voice.ui.xaml, 'Name="KwChipHost"') && InStr(voice.ui.xaml, 'Height="124"') && InStr(voice.ui.xaml, 'Padding="4,2,4,2"') && InStr(voice.ui.xaml, 'Name="KwChipPanel"') && InStr(voice.ui.xaml, 'Uid="ahk:Voice.Keywords.Chips"'), "voice chips sit in a 124px bordered scroller")
    Assert(voice.ui.rows.Length > 0 && InStr(voice.ui.rows[1], 'Margin="6,4,6,4"') && InStr(voice.ui.rows[1], 'Padding="10,6,10,6"') && InStr(voice.ui.rows[1], 'HorizontalAlignment="Right" VerticalAlignment="Top" Margin="0,-4,-8,0"'), "voice chips use 6px horizontal and 4px vertical spacing")
    Assert(InStr(voice.ui.xaml, "请输入宏触发关键词"), "voice input shows the trigger-keyword placeholder")
    Assert(InStr(voice.ui.xaml, 'Name="BtnKwMic"') && InStr(voice.ui.xaml, 'Name="BtnKwAdd"') && InStr(voice.ui.xaml, 'Content="+"'), "voice input has mic and plus-add buttons")
    Assert(InStr(voice.ui.xaml, 'Name="BtnSure"') && !InStr(voice.ui.xaml, 'Name="BtnCancel"'), "voice window keeps a centered confirm button")
    ...
    Assert(InStr(voice.ui.xaml, 'Name="KwInputHost"') && InStr(voice.ui.xaml, "{DynamicResource InputBg}") && InStr(voice.ui.xaml, "{DynamicResource ActionBg}"), "voice window copies theme input and action colors")
